...
function getDepartment(departments: SearchResult['departments']) {
  if (!departments) return null
  return Array.isArray(departments) ? departments[0] ?? null : departments
}
...
export function ClassesSearch({ initialSearch }: { initialSearch?: string }) {
  ...
  useEffect(() => {
    const searchClasses = async () => {
      ...
      // Step 1: find department IDs matching the search term
      ...
      // Step 2: search classes by name OR matching department_id
      let query = supabase
        .from('classes')
        .select(`id, name, departments (name, type)`)
        .limit(5)

      if (departmentIds.length > 0) {
        query = query.or(
          `name.ilike.%${search}%,department_id.in.(${departmentIds.join(',')})`
        )
      } else {
          ...
          {showDropdown && results.length > 0 && (
            <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-80 overflow-y-auto">
              ...
              {results.map((classItem) => {
                const dept = getDepartment(classItem.departments)
                return (
                  <button
                    key={classItem.id}
                    type="button"
